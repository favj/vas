...
namespace Tymetrix.T360.Mobile.Client.AppWP7.Invoice
{
    public partial class InvoiceCommonDetails : BasePage
    {
        ...
        private SummaryView GetDetailView(string label, string value)
        {
            ...
            view.DetailValue = (value.Length > 300) ? value.Substring(0, 300) + "..." : value;
            return view;
        }
        ...
        private void OnNotesClick(object parameter)
        {
            ...
            Uri uri = new Uri("/Tymetrix.T360.Mobile.Client.AppWP7;component/Invoice/NotesPage.xaml", UriKind.Relative);
        ...
        private void GetLineItemList(object sender, ServiceEventArgs args)
        {
            ...
            if (result.Status)
            {
                ...
                {
                    SummaryViewModel.OriginalLineItemList = JsonConvert.DeserializeObject<List<LineItem>>(result.Output);
                    SummaryViewModel.LineItemList = new List<LineItem>(SummaryViewModel.OriginalLineItemList);
        ...
        private void AdjustInvoice(object sender, ServiceEventArgs args)
        {
            ...
            if (result.Status)
            {
                ...
                Deployment.Current.Dispatcher.BeginInvoke(() =>
                {
                    this.ProgressBar.Show();
                    PageInProgress = true;
                    Uri uri = new Uri("/Tymetrix.T360.Mobile.Client.AppWP7;component/Invoice/AdjustPage.xaml", UriKind.Relative);
                    this.NavigationService.Navigate(uri);
                    this.ProgressBar.Hide();
                });
            }
